...
class Sampler(ABC):
    ...
    def __init__(self, backend: Backend = Backend.EMULATOR, 
                 config: Optional[TSUConfig] = None):
    ...
    def sample(self, n: int = 1000, return_metadata: bool = False) -> Union[np.ndarray, SamplingResult]:
        """
        ...
        if return_metadata:
            return SamplingResult(
                ...
                backend_used=str(self.backend.value)
            )
        return samples
...
class MultimodalSampler(Sampler):
    ...
    def __init__(self, centers: Union[List[np.ndarray], List[List]], weights: List[float], **kwargs):
        super().__init__(**kwargs)
...
class BayesianSampler(Sampler):
    ...
    def __init__(self, log_likelihood: Callable, log_prior: Callable,
                 *args, dim: Optional[int] = None, **kwargs):
        super().__init__(**kwargs)
...
def sample_gaussian(mu: float = 0, sigma: float = 1, n: int = 1000,
                   backend: Backend = Backend.EMULATOR) -> np.ndarray:
...
def sample_multimodal(centers: List, weights: List, n: int = 1000,
                     backend: Backend = Backend.EMULATOR) -> np.ndarray:
...
def compare_samplers(distribution, methods: List[str] = ['tsu', 'mcmc'],
                    n_samples: int = 1000) -> Dict[str, SamplingResult]:
    """
...
def optimize(problem: OptimizationProblem, method: str = 'tsu',
