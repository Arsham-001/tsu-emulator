...
def run_reliable_benchmark(
    n_trials: int = 10, n_samples: int = 500, dim: int = 10, verbose: bool = True
    ...
    for trial in range(n_trials):
        ...
        if verbose:
            tsu_modes_found = tsu_quality["modes_found"]
            mcmc_modes_found = mcmc_quality["modes_found"]
            print(
                f"TSU: {tsu_modes_found}/3 modes, "
                f"MCMC: {mcmc_modes_found}/3 modes → {winner} wins"
            )
...
def print_benchmark_summary(tsu_result: BenchmarkResult, mcmc_result: BenchmarkResult):
    ...
    # Modes found
    tsu_modes_info = f"{tsu_result.avg_modes_found:.2f} ± {tsu_result.std_modes_found:.2f}"
    mcmc_modes_info = f"{mcmc_result.avg_modes_found:.2f} ± {mcmc_result.std_modes_found:.2f}"
    print(f"{'Avg Modes Found':<30} {tsu_modes_info}     {mcmc_modes_info}")

    # Energy
    tsu_energy_info = f"{tsu_result.avg_energy:.2f} ± {tsu_result.std_energy:.2f}"
    mcmc_energy_info = f"{mcmc_result.avg_energy:.2f} ± {mcmc_result.std_energy:.2f}"
    print(f"{'Avg Best Energy':<30} {tsu_energy_info}     {mcmc_energy_info}")

    # Time
    tsu_time_info = f"{tsu_result.avg_time:.2f}s"
    mcmc_time_info = f"{mcmc_result.avg_time:.2f}s"
    print(f"{'Avg Execution Time':<30} {tsu_time_info}            {mcmc_time_info}")

    # Win rate
    tsu_win = f"{tsu_result.win_rate*100:.1f}%"
    mcmc_win = f"{mcmc_result.win_rate*100:.1f}%"
    print(f"\n{'Win Rate':<30} {tsu_win}              {mcmc_win}")
    ...
    if hasattr(test_result, "pvalue"):
        ...
        p_value = test_result[1]  # type: ignore
...
def full_benchmark():
    ...
    with open("benchmark_results.txt", "w") as f:
        f.write("TSU Platform Benchmark Results\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Trials: {tsu_result.n_trials}\n")
        f.write(f"TSU Win Rate: {tsu_result.win_rate*100:.1f}%\n")
        tsu_energy = f"{tsu_result.avg_energy:.2f} ± {tsu_result.std_energy:.2f}"
        f.write(f"TSU Avg Best Energy: {tsu_energy}\n")
        mcmc_energy = f"{mcmc_result.avg_energy:.2f} ± {mcmc_result.std_energy:.2f}"
        f.write(f"MCMC Avg Best Energy: {mcmc_energy}\n")
