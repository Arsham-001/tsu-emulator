...
class MultimodalDistribution:
    ...
    def sample_tsu(self, n_samples: int = 1000) -> tuple:
        ...
        for temp in temperatures:
            config = TSUConfig(temperature=temp, n_steps=300, n_burnin=100)
...
def create_plotly_mode_visualization(
    tsu_samples, mcmc_samples, dist, save_path="tsu_modes_2d.html"
):
    """
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            name="Classical MCMC",
            marker=dict(size=4, color="#e74c3c", opacity=0.3, line=dict(width=0)),
            hoverinfo="skip",
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            name="TSU Emulator",
            marker=dict(size=5, color="#2ecc71", opacity=0.5, line=dict(width=0)),
            hovertemplate=(
                "<b>TSU Sample</b><br>Dim 1: %{x:.2f}<br>Dim 2: %{y:.2f}<extra></extra>"
            ),
        )
    )
    ...
    for i in range(len(mode_x)):
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    f"<b>Mode {i+1}</b><br>Weight: {dist.mode_weights[i]:.2f}<br>"
                    "Dim 1: %{x:.2f}<br>Dim 2: %{y:.2f}<extra></extra>"
                ),
            )
        )
    ...
    fig.update_layout(
        title={
            "text": (
                "<b>Sampling Distribution in 2D Mode Space</b><br>"
                "<sub>TSU vs Classical MCMC</sub>"
            ),
            "x": 0.5,
            "xanchor": "center",
            "font": {"size": 18},
...
def create_plotly_energy_comparison(
    tsu_quality, mcmc_quality, save_path="tsu_energy_comparison_2d.html"
):
    """
...
def analyze_results(
    ...
    tsu_mean = tsu_quality["mean_energy"]
    mcmc_mean = mcmc_quality["mean_energy"]
    print(f"{'Mean Energy':<25} {tsu_mean:<20.4f} {mcmc_mean:<20.4f}")
    tsu_std = tsu_quality["std_energy"]
    mcmc_std = mcmc_quality["std_energy"]
    print(f"{'Std Deviation':<25} {tsu_std:<20.4f} {mcmc_std:<20.4f}")
    tsu_min = tsu_quality["min_energy"]
    mcmc_min = mcmc_quality["min_energy"]
    print(f"{'Min Energy':<25} {tsu_min:<20.4f} {mcmc_min:<20.4f}")

    # Energy difference
    mean_energy_diff = mcmc_quality["mean_energy"] - tsu_quality["mean_energy"]
    mean_energy_improvement = (
        (mean_energy_diff / abs(mcmc_quality["mean_energy"])) * 100
        if mcmc_quality["mean_energy"] != 0
        else 0
    )
    print(
        f"{'Mean Energy Diff':<25} {mean_energy_diff:<20.4f} ({mean_energy_improvement:+.1f}%)"
    )
    ...
    print(f"{'Metric':<25} {'TSU':<20} {'MCMC':<20}")
    print("-" * 80)
    tsu_modes = tsu_quality["modes_found"]
    mcmc_modes = mcmc_quality["modes_found"]
    print(f"{'Modes Found':<25} {tsu_modes}/3{' ':<13} {mcmc_modes}/3")
    ...
    for mode_idx in range(len(dist.mode_centers)):
        ...
        tsu_near = tsu_mode["samples_near"]
        tsu_dist = tsu_mode["min_distance"]
        print(f"    TSU:  {tsu_near:3d} samples near, min dist={tsu_dist:6.3f}")
        mcmc_near = mcmc_mode["samples_near"]
        mcmc_dist = mcmc_mode["min_distance"]
        print(f"    MCMC: {mcmc_near:3d} samples near, min dist={mcmc_dist:6.3f}")
    ...
    tsu_cov_pct = tsu_coverage * 100
    mcmc_cov_pct = mcmc_coverage * 100
    print(
        f"{'Mode Coverage Efficiency':<25} {tsu_cov_pct:>6.1f}%{' ':<7} {mcmc_cov_pct:>6.1f}%"
    )
    tsu_e = tsu_quality["mean_energy"]
    mcmc_e = mcmc_quality["mean_energy"]
    label = "Energy Quality (lower is better)"
    print(f"{label:<25} {tsu_e:>10.2f}{' ':<3} {mcmc_e:>10.2f}")
    ...
    if tsu_wins_energy:
        tsu_min_e = tsu_quality["min_energy"]
        mcmc_min_e = mcmc_quality["min_energy"]
        print(f"[OK] TSU found lower minimum energy: {tsu_min_e:.3f} vs {mcmc_min_e:.3f}")
    if tsu_wins_modes:
        tsu_m = tsu_quality["modes_found"]
        mcmc_m = mcmc_quality["modes_found"]
        print(f"[OK] TSU found more modes: {tsu_m}/3 vs {mcmc_m}/3")
    if tsu_wins_spread:
        tsu_sd = tsu_quality["std_energy"]
        mcmc_sd = mcmc_quality["std_energy"]
        print(f"[OK] TSU has better energy distribution: σ={tsu_sd:.3f} vs {mcmc_sd:.3f}")

    if wins == 0:
        print(
            "[WARNING] Results are comparable between TSU and classical MCMC on this instance"
        )
    elif wins == 3:
    ...
    print("\n[6] RESEARCH INSIGHTS")
    print("-" * 80)
    print("• Multimodal distribution in 10D space with 3 modes")
    print(f"• Sample size: {len(tsu_samples)} samples per method")
    print("• TSU strategy: Parallel tempering with T∈[0.5, 1.0, 2.0]")
    print("• Classical strategy: Metropolis-Hastings with adaptive step size")
    hw_speedup = tsu_time / tsu_hardware_time
    print(f"• Hardware projection: {hw_speedup:.0e}x speedup achievable with TSU hardware")
    print("• Key advantage: TSU natural parallelism at physics level enables ensemble sampling")
...
def demo_continuous_sampling():
    ...
    try:
        ...
        print("         (3 modes with different weights)")
        ...
        tsu_samples, tsu_emulator_time, tsu_hardware_time, hw_explain = dist.sample_tsu(
            n_samples
        )
        ...
        # Determine winner
        tsu_better = (
            tsu_quality["modes_found"] > mcmc_quality["modes_found"]
            or tsu_quality["min_energy"] < mcmc_quality["min_energy"]
        )
        ...
        if tsu_better:
            ...
            tsu_modes = tsu_quality["modes_found"]
            mcmc_modes = mcmc_quality["modes_found"]
            print(f"   Found {tsu_modes} modes vs {mcmc_modes} for MCMC")
            if tsu_quality["min_energy"] < mcmc_quality["min_energy"]:
                improvement = (
                    (mcmc_quality["min_energy"] - tsu_quality["min_energy"])
                    / abs(mcmc_quality["min_energy"])
                    * 100
                )
        ...
        print("\nKey Insight:")
        print("  TSU explores via natural thermal fluctuations")
        print("  Hardware provides both algorithmic AND speed advantage")
        speedup = tsu_emulator_time / tsu_hardware_time
        print(f"  Emulator proves algorithm works, hardware adds {speedup:.0f}x speedup")
        print("=" * 70)
        ...
        create_plotly_mode_visualization(
            tsu_samples, mcmc_samples, dist, save_path="tsu_modes_research.html"
        )
        create_plotly_energy_comparison(
            tsu_quality, mcmc_quality, save_path="tsu_metrics_research.html"
        )
        ...
        plt.subplot(1, 3, 1)
        plt.scatter(
            tsu_samples[:, 0], tsu_samples[:, 1], alpha=0.5, s=10, c="green", label="TSU"
        )
        plt.scatter(
            dist.mode_centers[:, 0],
            dist.mode_centers[:, 1],
        ...
        plt.subplot(1, 3, 2)
        plt.scatter(
            mcmc_samples[:, 0], mcmc_samples[:, 1], alpha=0.5, s=10, c="red", label="MCMC"
        )
        plt.scatter(
            dist.mode_centers[:, 0],
            dist.mode_centers[:, 1],
