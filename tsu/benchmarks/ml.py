...
from ..ml import BayesianRegressor, BayesianNetwork
...
class MLBenchmark:
    ...
    def benchmark_regression_synthetic(self, n_train: int = 200,
                                      n_test: int = 100,
                                      n_folds: int = 5,
                                      noise_std: float = 0.1) -> MLResult:
        """
    ...
    def benchmark_regression_nonlinear(self, n_train: int = 300,
                                      n_test: int = 100,
                                      n_folds: int = 5) -> MLResult:
        """
        ...
        for fold in range(n_folds):
            ...
            coverage = np.mean((y_test >= predictions['lower']) & 
    ...
    def benchmark_extrapolation(self, n_train: int = 150,
                                n_test: int = 100,
                                n_folds: int = 5) -> MLResult:
        """
    ...
    def _compute_calibration_error(self, y_true: np.ndarray,
                                   y_pred: np.ndarray,
                                   y_std: np.ndarray,
                                   n_bins: int = 10) -> float:
        """
    ...
    def run_all_benchmarks(self, quick: bool = False) -> Dict[str, MLResult]:
        ...
        print(f"Test MSE: {summary['test_error']['mean']:.4f} ± {summary['test_error']['std']:.4f}")
