...
class ComparisonBenchmark:
    ...
    def compare_sampling_methods(self, n_samples: int = 5000,
                                n_trials: int = 5,
                                dim: int = 10) -> ComparisonResult:
        """
    ...
    def compare_optimization_methods(self, n_spins: int = 20,
                                    n_trials: int = 5,
                                    time_budget: float = 1.0) -> ComparisonResult:
        """
    ...
    def _metropolis_hastings_binary(self, n_samples: int, dim: int,
                                    burnin: int = 100) -> np.ndarray:
        """
    ...
    def _gaussian_kl(self, samples: np.ndarray, 
    ...
    def run_all_comparisons(self, quick: bool = False) -> Dict[str, ComparisonResult]:
        ...
        for framework in ['TSU', 'Direct', 'Metropolis']:
            ...
            print(f"{framework:12s}: KL={fw['objective']['mean']:.4f}, "
        ...
        results['optimization'] = self.compare_optimization_methods(
            n_spins=15 if quick else 20,
            n_trials=n_trials,
            time_budget=0.5 if quick else 1.0
        )
        summary = results['optimization'].summary()
        for framework in ['TSU', 'Random', 'Greedy']:
            fw = summary['frameworks'][framework]
            print(f"{framework:12s}: Best={fw['objective']['best']:.2f}, "
...
if __name__ == "__main__":
    ...
    for name, result in results.items():
        ...
        for framework, metrics in summary['frameworks'].items():
            print(f"\n  {framework}:")
            print(f"    Objective: {metrics['objective']['mean']:.4f} ± {metrics['objective']['std']:.4f}")
