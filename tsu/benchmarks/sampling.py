...
class SamplingBenchmark:
    ...
    def benchmark_gaussian(self, n_samples: int = 10000, 
                          n_trials: int = 5,
                          dim: int = 1) -> SamplingResult:
        """
        ...
        for trial in range(n_trials):
            ...
            samples = self.sampler.sample_boltzmann(
                J, bias=h, n_samples=n_samples
            )
    ...
    def benchmark_boltzmann(self, n_spins: int = 10,
                           n_samples: int = 10000,
                           n_trials: int = 5) -> SamplingResult:
        """
        ...
        """
        result = SamplingResult(
            distribution_name=f"Boltzmann(n={n_spins})",
            n_samples=n_samples,
            n_trials=n_trials
        )
    ...
    def benchmark_multimodal(self, n_samples: int = 10000,
                            n_trials: int = 5) -> SamplingResult:
        """
        ...
        """
        result = SamplingResult(
            distribution_name="Ferromagnetic_Bimodal",
            n_samples=n_samples,
            n_trials=n_trials
        )
    ...
    def _estimate_kl_divergence(self, samples: np.ndarray, 
                               true_pdf: Callable) -> float:
        """
    ...
    def _compute_boltzmann_energies(self, samples: np.ndarray,
                                    J: np.ndarray, h: np.ndarray) -> np.ndarray:
        """
    ...
    def run_all_benchmarks(self, quick: bool = False) -> Dict[str, SamplingResult]:
        ...
        print(f"KS test pass rate: {np.mean(np.array(results['gaussian_1d'].ks_pvalues) > 0.05):.2%}")
        ...
        print(f"Effective sample size: {np.mean(results['boltzmann'].effective_sample_sizes):.0f}")
...
if __name__ == "__main__":
    ...
    for name, result in results.items():
        ...
        print(f"  KL Divergence: {summary['kl_divergence']['mean']:.4f} ± {summary['kl_divergence']['std']:.4f}")
        print(f"  ESS: {summary['effective_sample_size']['mean']:.0f} ± {summary['effective_sample_size']['std']:.0f}")
