...
class IsingModel:
    ...
    def sample(
        self, n_samples: int = 1000, initial_state: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """
        ...
        bit_samples = self.sampler.sample_boltzmann(
            J_bit, bias=h_bit, n_samples=n_samples, initial_state=initial_bits
        )
...
class IsingChain(IsingModel):
    ...
    def __init__(self, n_spins: int, J: float = 1.0, config: Optional[IsingConfig] = None):
        """
...
class IsingGrid(IsingModel):
    ...
    def visualize(
        self, state: np.ndarray, title: str = "Ising Grid", cmap: str = "RdBu_r"
    ) -> Figure:
        """
...
def demonstrate_phase_transition(
    sizes: List[int] = [8, 16, 32], temperatures: Optional[np.ndarray] = None
) -> dict:
    """
