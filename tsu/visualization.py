...
try:
    ...
    warnings.warn(
        "Plotly not available. Install with 'pip install plotly' for interactive plots."
    )
...
def plot_predictions_with_uncertainty(
    ...
    if y_true is not None:
        ...
        ax.plot(x_sorted, y_true_sorted, "k--", linewidth=1.5, label="True function", alpha=0.7)

    # Training data if provided
    if x_train is not None and y_train is not None:
        ax.scatter(x_train, y_train, c="red", s=30, alpha=0.6, label="Training data", zorder=5)
...
def plot_uncertainty_vs_error(
    ...
    ax1.scatter(uncertainties, errors, alpha=0.5, s=20, c=uncertainties, cmap=UNCERTAINTY_CMAP)
    ax1.plot(
        [0, uncertainties.max()],
        [0, uncertainties.max()],
        "k--",
    ...
    ax2.errorbar(
        bin_centers, bin_errors, yerr=bin_stds, fmt="o-", capsize=5, capthick=2, linewidth=2
    )
    ax2.plot(
        [0, bin_centers[-1]],
        [0, bin_centers[-1]],
...
def plot_energy_landscape_2d(
    ...
    ax.contour(X, Y, Z, levels=levels, linewidths=0.5, alpha=0.4)
    ...
    if samples is not None:
        ax.scatter(
            samples[:, 0], samples[:, 1], c="red", s=10, alpha=0.5, label="Samples", zorder=3
        )

    # Overlay trajectory if provided
    if trajectory is not None:
        ax.plot(
            trajectory[:, 0], trajectory[:, 1], "w-", linewidth=2, alpha=0.8, label="Trajectory"
        )
        ax.scatter(
            trajectory[0, 0],
            trajectory[0, 1],
...
def plot_ising_state(
    ...
    if state.ndim == 1:
        ...
        im = ax.imshow(spins, cmap=DIVERGING_CMAP, vmin=-1, vmax=1, interpolation="nearest")
...
def plot_phase_transition(
    ...
    if magnetization_errors is not None:
        ...
        ax.plot(temperatures, magnetizations, "o-", linewidth=2, markersize=8, label="Measured")
    ...
    if critical_temp is not None:
        ax.axvline(
            ...
            linewidth=2,
            alpha=0.7,
            label=f"Critical T_c={critical_temp:.3f}",
        )
...
def plot_sampling_diagnostics(
    ...
    if true_distribution is not None:
        ...
        axes[0].plot(
            x_range,
            [true_distribution(x) for x in x_range],
            "r-",
            linewidth=2,
    ...
    lags = range(max_lag)
    autocorr = [
        np.corrcoef(sample_1d[: -lag or None], sample_1d[lag:])[0, 1] if lag > 0 else 1.0
        for lag in lags
    ]

    axes[1].plot(lags, autocorr, "b-", linewidth=2)
    axes[1].axhline(0, color="k", linestyle="--", alpha=0.3)
    axes[1].axhline(0.05, color="r", linestyle=":", alpha=0.5, label="5% threshold")
...
def plot_active_learning_curve(
    ...
    ax.fill_between(
        ...
        alpha=0.2,
        color="blue",
        label="Active learning advantage",
    )
...
def plot_interactive_energy_landscape(
    ...
    fig.update_layout(
        title=title,
        scene=dict(xaxis_title="x₁", yaxis_title="x₂", zaxis_title="Energy"),
        width=800,
        height=600,
    )
