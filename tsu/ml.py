...
from .gibbs import GibbsSampler, GibbsConfig  # noqa: F401
...
class StochasticLayer(ABC):
    ...
    def compute_kl_divergence(self) -> float:
        ...
        weight_kl = 0.5 * np.sum(
            ...
            + (self.weight_std**2 + self.weight_mean**2) / self.prior_std**2
            - 1
        )
        ...
        bias_kl = 0.5 * np.sum(
            ...
            + (self.bias_std**2 + self.bias_mean**2) / self.prior_std**2
            - 1
        )
...
class BayesianNetwork:
    ...
    def __init__(
        ...
        for i in range(len(layer_sizes) - 1):
            layer = BayesianLinear(layer_sizes[i], layer_sizes[i + 1], prior_std=prior_std)
    ...
    def predict(self, x: np.ndarray, n_samples: int = 100) -> PredictionResult:
        ...
        for _ in range(n_samples):
            ...
            weights_list = [layer.sample_weights(self.temperature) for layer in self.layers]
        ...
        return PredictionResult(mean=mean, std=std, samples=predictions)
    ...
    def fit(
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                for _ in range(n_samples_per_batch):
                    # Sample weights
                    weights_list = [
                        layer.sample_weights(self.temperature) for layer in self.layers
                    ]
                    ...
                    self._backward_pass(
                        x_batch, y_batch, weights_list, learning_rate, kl_weight
                    )
        ...
        return {"loss_history": self.loss_history, "kl_history": self.kl_history}
...
class BayesianRegressor(BayesianNetwork):
    ...
    def __init__(
        ...
        super().__init__(
            layer_sizes, activation="relu", prior_std=prior_std, temperature=temperature
        )

    def predict_with_interval(
        self, x: np.ndarray, n_samples: int = 100, confidence: float = 0.95
    ) -> dict:
    ...
    def select_informative_samples(
        self, x_pool: np.ndarray, n_select: int, n_samples: int = 100
    ) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    history = model.fit(
        x_train, y_train, n_epochs=50, batch_size=32, learning_rate=0.01, verbose=False
    )
